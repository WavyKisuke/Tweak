...
static BOOL IsTikTokAudio(void) { return [[[NSBundle mainBundle] bundleIdentifier] isEqualToString:@"com.zhiliaoapp.musically"]; }
...
static void DebugLog(NSString *fmt, ...) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSArray *lines = nil; @synchronized (gDebugLines) { lines = [gDebugLines copy]; }
...
static void TrackPlayer(AVPlayer *p) { if (!p || !gTrackedPlayers) return; @synchronized(gTrackedPlayers) { [gTrackedPlayers addObject:p]; } DebugLog(@"PATH AVPlayer tracked"); }
static void TrackAudioPlayer(AVAudioPlayer *p) { if (!p || !gTrackedAudioPlayers) return; @synchronized(gTrackedAudioPlayers) { [gTrackedAudioPlayers addObject:p]; } DebugLog(@"PATH AVAudioPlayer tracked"); }
static void TrackEngine(AVAudioEngine *e) { if (!e || !gTrackedEngines) return; @synchronized(gTrackedEngines) { [gTrackedEngines addObject:e]; } DebugLog(@"PATH AVAudioEngine tracked"); }
static void TrackPlayerNode(AVAudioPlayerNode *n) { if (!n || !gTrackedPlayerNodes) return; @synchronized(gTrackedPlayerNodes) { [gTrackedPlayerNodes addObject:n]; } DebugLog(@"PATH AVAudioPlayerNode tracked"); }
static void TrackMixerNode(AVAudioMixerNode *n) { if (!n || !gTrackedMixerNodes) return; @synchronized(gTrackedMixerNodes) { [gTrackedMixerNodes addObject:n]; } DebugLog(@"PATH AVAudioMixerNode tracked"); }
static void TrackEnvironmentNode(AVAudioEnvironmentNode *n) { if (!n || !gTrackedEnvironmentNodes) return; @synchronized(gTrackedEnvironmentNodes) { [gTrackedEnvironmentNodes addObject:n]; } DebugLog(@"PATH AVAudioEnvironmentNode tracked"); }
static void TrackSampleRenderer(AVSampleBufferAudioRenderer *r) { if (!r || !gTrackedSampleRenderers) return; @synchronized(gTrackedSampleRenderers) { [gTrackedSampleRenderers addObject:r]; } DebugLog(@"PATH AVSampleBufferAudioRenderer tracked"); }
...
- (void)tapMute:(id)sender { gTikTokMuted = !gTikTokMuted; DebugLog(@"UI MUTE tapped -> %@", gTikTokMuted ? @"ON" : @"OFF"); [gMuteButton setTitle:gTikTokMuted ? @"UNMUTE" : @"MUTE" forState:UIControlStateNormal]; ApplyMuteState(); }
- (void)tapDebug:(id)sender { dispatch_async(dispatch_get_main_queue(), ^{ if (!gDebugPanel) return; gDebugPanel.hidden = !gDebugPanel.hidden; if (!gDebugPanel.hidden && gDebugView.text.length) [gDebugView scrollRangeToVisible:NSMakeRange(gDebugView.text.length - 1, 1)]; }); }
...
static void InstallDebugOverlay(UIWindow *w) {
    ...
    UIButton *b = [UIButton buttonWithType:UIButtonTypeSystem]; b.frame = CGRectMake(w.bounds.size.width - 102, 104, 88, 34); b.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin; b.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:.68]; b.layer.cornerRadius = 9; [b setTitle:@"DEBUG" forState:UIControlStateNormal]; [b setTitleColor:UIColor.whiteColor forState:UIControlStateNormal]; b.titleLabel.font = [UIFont boldSystemFontOfSize:13]; b.accessibilityIdentifier = @"TikTokPlusDebugButton"; [b addTarget:gAudioTarget action:@selector(tapDebug:) forControlEvents:UIControlEventTouchUpInside]; [w addSubview:b]; gDebugButton = b;
    CGFloat width = MIN(w.bounds.size.width - 24, 520);
    UIView *panel = [[UIView alloc] initWithFrame:CGRectMake(12, 145, width, MIN(w.bounds.size.height - 170, 420))]; panel.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight; panel.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:.92]; panel.layer.cornerRadius = 12; panel.hidden = YES;
    UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(12, 8, width - 70, 28)]; title.text = @"TikTokPlus Audio Diagnostics"; title.textColor = UIColor.whiteColor; title.font = [UIFont boldSystemFontOfSize:14]; [panel addSubview:title];
    UIButton *close = [UIButton buttonWithType:UIButtonTypeSystem]; close.frame = CGRectMake(width - 48, 5, 40, 34); [close setTitle:@"×" forState:UIControlStateNormal]; [close setTitleColor:UIColor.whiteColor forState:UIControlStateNormal]; close.titleLabel.font = [UIFont boldSystemFontOfSize:24]; [close addTarget:gAudioTarget action:@selector(tapCloseDebug:) forControlEvents:UIControlEventTouchUpInside]; [panel addSubview:close];
    UITextView *tv = [[UITextView alloc] initWithFrame:CGRectMake(8, 42, width - 16, panel.bounds.size.height - 50)]; tv.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight; tv.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:.35]; tv.textColor = UIColor.greenColor; tv.font = [UIFont monospacedSystemFontOfSize:11 weight:UIFontWeightRegular]; tv.editable = NO; tv.selectable = YES; tv.text = @"Waiting for audio events..."; [panel addSubview:tv]; [w addSubview:panel]; gDebugPanel = panel; gDebugView = tv; DebugLog(@"DEBUG OVERLAY READY");
}

static void InstallMuteButton(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIWindow *w = AudioTopWindow(); if (!w) return; if (!gAudioTarget) gAudioTarget = [TTKPlusAudioTarget new];
        if (!gMuteButton) { UIButton *b = [UIButton buttonWithType:UIButtonTypeSystem]; b.frame = CGRectMake(w.bounds.size.width - 102, 60, 88, 38); b.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin; b.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:.68]; b.layer.cornerRadius = 10; [b setTitle:gTikTokMuted ? @"UNMUTE" : @"MUTE" forState:UIControlStateNormal]; [b setTitleColor:UIColor.whiteColor forState:UIControlStateNormal]; b.titleLabel.font = [UIFont boldSystemFontOfSize:14]; b.accessibilityIdentifier = @"TikTokPlusMuteButton"; [b addTarget:gAudioTarget action:@selector(tapMute:) forControlEvents:UIControlEventTouchUpInside]; [w addSubview:b]; gMuteButton = b; DebugLog(@"UI MUTE BUTTON READY"); }
...
%ctor {
    ...
    gTrackedPlayers=[NSHashTable weakObjectsHashTable]; gTrackedAudioPlayers=[NSHashTable weakObjectsHashTable]; gTrackedEngines=[NSHashTable weakObjectsHashTable]; gTrackedPlayerNodes=[NSHashTable weakObjectsHashTable]; gTrackedMixerNodes=[NSHashTable weakObjectsHashTable]; gTrackedEnvironmentNodes=[NSHashTable weakObjectsHashTable]; gTrackedSampleRenderers=[NSHashTable weakObjectsHashTable]; gDebugLines=[NSMutableArray array];
    DebugLog(@"TTKPLUS AUDIO DIAGNOSTICS INITIALIZED"); dispatch_async(dispatch_get_main_queue(), ^{ InstallMuteButton(); });
    gMuteTimer=dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER,0,0,dispatch_get_main_queue()); dispatch_source_set_timer(gMuteTimer,dispatch_time(DISPATCH_TIME_NOW,750000000),750000000,100000000); dispatch_source_set_event_handler(gMuteTimer,^{if(gTikTokMuted)ApplyMuteState();}); dispatch_resume(gMuteTimer);
}
