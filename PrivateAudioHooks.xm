...
static BOOL gPrivateApplying=NO;
...
static void PTCallBoolOriginal(id o,SEL sel,BOOL value){IMP p=PTOriginalForSelf(o,sel);if(p)((void(*)(id,SEL,BOOL))p)(o,sel,value);}
static void PTCallFloatOriginal(id o,SEL sel,float value){IMP p=PTOriginalForSelf(o,sel);if(p)((void(*)(id,SEL,float))p)(o,sel,value);}

// Never send hooked setters from the enforcement path. Use the saved original
// IMP directly so the private-hook layer cannot recursively call itself.
static void PTForceObject(id o){
    if(!o||!gPrivateTikTokMuted)return;
    BOOL wasApplying=gPrivateApplying;gPrivateApplying=YES;
    @try{
        if([o respondsToSelector:@selector(setMuted:)])PTCallBoolOriginal(o,@selector(setMuted:),YES);
        if([o respondsToSelector:@selector(setAudioMuted:)])PTCallBoolOriginal(o,@selector(setAudioMuted:),YES);
        if([o respondsToSelector:@selector(setEnableSoundOutput:)])PTCallBoolOriginal(o,@selector(setEnableSoundOutput:),NO);
        if([o respondsToSelector:@selector(setVolume:)])PTCallFloatOriginal(o,@selector(setVolume:),0.0f);
        if([o respondsToSelector:@selector(setAudioVolume:)])PTCallFloatOriginal(o,@selector(setAudioVolume:),0.0f);
        if([o respondsToSelector:@selector(setPlayerVolume:)])PTCallFloatOriginal(o,@selector(setPlayerVolume:),0.0f);
        if([o respondsToSelector:@selector(setOutputVolume:)])PTCallFloatOriginal(o,@selector(setOutputVolume:),0.0f);
    }@catch(__unused NSException *e){}
    gPrivateApplying=wasApplying;
}
...
static void PTRestoreObject(id o){
    ...
    BOOL wasApplying=gPrivateApplying;gPrivateApplying=YES;
    @try{
        if([o respondsToSelector:@selector(setMuted:)])PTCallBoolOriginal(o,@selector(setMuted:),NO);
        if([o respondsToSelector:@selector(setAudioMuted:)])PTCallBoolOriginal(o,@selector(setAudioMuted:),NO);
        if([o respondsToSelector:@selector(setEnableSoundOutput:)])PTCallBoolOriginal(o,@selector(setEnableSoundOutput:),YES);
    }@catch(__unused NSException *e){}
    gPrivateApplying=wasApplying;
}
...
static void PTProbePlayer(id player){
    if(!player)return;
    PTProbeNode(player);
    for(NSString*n in @[@"player",@"currentPlayer",@"audioPlayer",@"avPlayer",@"videoPlayer",@"playerNode",@"mixerNode",@"silentNode",@"audioEngine",@"audioMixerNode",@"mainMixerNode",@"audioRenderer",@"audioOutput",@"soundPlayer",@"soundEngine",@"core",@"audioCore",@"playerCore",@"audioPlayerCore"]){
        SEL s=NSSelectorFromString(n);if(![player respondsToSelector:s])continue;
        @try{id child=((id(*)(id,SEL))objc_msgSend)(player,s);if(child&&child!=player)PTProbeNode(child);}@catch(__unused NSException*e){}
    }
}
...
static void PTSetBool(id self,SEL sel,BOOL value){
    if(gPrivateTikTokMuted&&!gPrivateApplying)value=YES;
...
static void PTSetFloat(id self,SEL sel,float value){
    if(gPrivateTikTokMuted&&!gPrivateApplying)value=0.0f;
...
static void PTSetDouble(id self,SEL sel,double value){
    if(gPrivateTikTokMuted&&!gPrivateApplying)value=0.0;
...
static void PTEnableSound(id self,SEL sel,BOOL value){
    if(gPrivateTikTokMuted&&!gPrivateApplying)value=NO;
    IMP o=PTOriginalForSelf(self,sel);if(o)((void(*)(id,SEL,BOOL))o)(self,sel,value);
}
...
static void InstallPrivateHooks(void){
    ...
    NSArray *extraWrappers=@[
        @"TTKPlayerView",@"TTKPlayerViewController",@"TTKFeedPlayerController",@"TTKMediaPlayerController",
        @"TTKPlayerManager",@"TTKPlayerContainer",@"TTKPlayerWrapper",@"TTKPlayerItem",@"TTKFeedCellPlayer",
        @"TTKFeedPlayerView",@"TTKFeedPlayerManager",@"TTKAudioEngine",@"TTKAudioPlayer",@"TTKAudioMix",
        @"TTKAudioManager",@"TTKAudioRenderer",@"TTKAudioSession",@"TTKAudioMixer",@"TTKVolumeController",
        @"TTKVolumeHandler",@"BDXAudioEngine",@"BDXAudioPlayer",@"BDXAudioManager",@"IESAudioEngine",
        @"IESAudioPlayer",@"IESAudioMix",@"IESAudioMixer",@"IESAudioRenderer",@"IESAudioSession",
        @"IESVolumeHandler",@"IESVideoPlayerController",@"IESVideoPlayer",@"BDXLynxVideoPlayerPro",
        @"BDXLynxAudioPlayer",@"TTKECMMKVideoPlayer",@"TTKECMMKAudioPlayer",@"IESMMBGAVPlayer",
        @"IESMMBGVideoPlayer",@"VEEffectVideoPlayer",@"VEEffectAudioPlayer",@"AWEPlayAudioManager",
        @"AWEVolumeHandler",@"AWEVolumeController",@"AWEFeedPlayerView",@"AWEFeedPlayerController",
        @"AWEPlayerWrapper",@"AWEPlayerManager",@"HTSAudioRenderer",@"HTSAudioPlayer",@"HTSAudioEngine",
        @"TTKLivePlayer",@"TTKLiveAudioPlayer",@"TTKLiveAudioRenderer",@"AWELivePlayer",
        @"AWELivePlayerController",@"AWELiveAudioRenderer",@"TPAudioPlayer",@"TPAudioEngine",
        @"TPAudioRenderer",@"BDPlayer",@"BDVideoPlayer",@"BDAudioPlayer",@"HPSAudioPlayer",@"HPSAudioEngine"
    ];
    for(NSString *name in extraWrappers)InstallWrapperHooks(NSClassFromString(name));

    // Targeted AVAudioEngine coverage. We intentionally do not hook every
    // Objective-C class in the process: a catch-all setter hook can hit
    // unrelated objects and, worse, misclassify BOOL setters as float/double
    // setters. That is a crash risk and can affect non-TikTok audio.
    ...
    dispatch_once(&once, ^{
        ...
        IMP replacement=imp_implementationWithBlock(^(id self){
            ...
            if(PTIsTikTok()&&e&&gPrivateTikTokMuted){
                @try{if([e respondsToSelector:@selector(mainMixerNode)])[[e mainMixerNode] setOutputVolume:0.0f];}@catch(__unused NSException *ex){}
            }
            return e;
        });
...
static void PTRestoreCurrent(void){
    ...
    if([gCurrentPlayerController respondsToSelector:@selector(player)]){@try{PTRestoreObject(((id(*)(id,SEL))objc_msgSend)(gCurrentPlayerController,@selector(player)));}@catch(__unused NSException*e){}}
    if([gCurrentPlayerController respondsToSelector:@selector(currentPlayer)]){@try{PTRestoreObject(((id(*)(id,SEL))objc_msgSend)(gCurrentPlayerController,@selector(currentPlayer)));}@catch(__unused NSException*e){}}
}
...
%ctor{
    ...
    dispatch_async(dispatch_get_main_queue(),^{
        [[NSNotificationCenter defaultCenter]addObserverForName:@"TikTokPlusMuteChanged" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification*n){
            NSNumber *m=n.userInfo[@"muted"];if(m)gPrivateTikTokMuted=m.boolValue;
            if(gPrivateTikTokMuted)PTApplyCurrent();else PTRestoreCurrent();
        }];
    });
