...
static NSHashTable *gSeenObjects;
...
static void PTProbeNode(id node){
    if(!node||node==gCurrentPlayer)return;
    if([gSeenObjects containsObject:node])return;
    [gSeenObjects addObject:node];
 ...
 * This is intentionally shallow and selector-driven: it reaches the actual
 * AVPlayer/audio node without recursively walking arbitrary UIKit objects. */
static void PTProbePlayer(id player){
    if(!player)return;
    PTProbeNode(player);
    NSArray*names=@[@"player",@"currentPlayer",@"audioPlayer",@"avPlayer",@"videoPlayer",@"playerNode",@"mixerNode",@"silentNode",@"audioEngine",@"audioMixerNode",@"mainMixerNode",@"audioRenderer",@"audioOutput",@"soundPlayer",@"soundEngine",@"core",@"audioCore",@"playerCore",@"audioPlayerCore"];
    for(NSString*n in names){SEL s=NSSelectorFromString(n);if(![player respondsToSelector:s])continue;@try{id child=((id(*)(id,SEL))objc_msgSend)(player,s);if(child&&child!=player)PTProbeNode(child);}@catch(__unused NSException*e){}}
...
/* Capture object-valued player assignment methods. These are important when
 * the wrapper is constructed before the feed starts playback. */
...
static void InstallPrivateHooks(void){
    ...
    if(controller||cell){
        if(controller){PTInstall(controller,@selector(playerWillLoopPlaying:),(IMP)PTPlayerLoop);PTInstall(controller,@selector(play),(IMP)PTControllerPlay);}
        if(cell){PTInstall(cell,@selector(playerWillLoopPlaying:),(IMP)PTPlayerLoop);PTInstall(cell,@selector(containerDidFullyDisplayWithReason:),(IMP)PTFeedDisplay);}
        for(Class c in @[controller?:Nil,cell?:Nil])if(c){
            for(NSString*n in @[@"setMuted:",@"setAudioMuted:",@"setEnableSoundOutput:"]){PTInstall(c,NSSelectorFromString(n),(IMP)PTSetBool);}
            for(NSString*n in @[@"setVolume:",@"setAudioVolume:",@"setPlayerVolume:",@"setOutputVolume:"]){SEL s=NSSelectorFromString(n);Method m=class_getInstanceMethod(c,s);if(m){const char*t=method_getTypeEncoding(m);PTInstall(c,s,(t&&strchr(t,'d'))?(IMP)PTSetDouble:(IMP)PTSetFloat);}}
            for(NSString*n in @[@"setPlayer:",@"setCurrentPlayer:",@"setAVPlayer:",@"setAvPlayer:",@"setAudioPlayer:",@"setVideoPlayer:"]){PTInstall(c,NSSelectorFromString(n),(IMP)PTCaptureObject);}
        }
    }
...
static void PTApplyCurrent(void){
    ...
    PTForceObject(gCurrentPlayer);PTForceObject(gCurrentPlayerController);PTForceObject(gCurrentFeedCell);
...
%ctor{
    ...
    gOriginalIMPs=[NSMutableDictionary dictionary];gSeenObjects=[NSHashTable weakObjectsHashTable];
