...
static void PTMuteObject(id obj) {
    if(!obj)return;
    BOOL touched=NO;
    touched |= PTSetBool(obj,@selector(setMuted:),gPrivateTikTokMuted);
    touched |= PTSetBool(obj,@selector(setMute:),gPrivateTikTokMuted);
    touched |= PTSetBool(obj,@selector(setAudioMuted:),gPrivateTikTokMuted);
    if(gPrivateTikTokMuted){
        touched |= PTSetFloat(obj,@selector(setVolume:),0.0f);
        touched |= PTSetFloat(obj,@selector(setAudioVolume:),0.0f);
        touched |= PTSetFloat(obj,@selector(setPlayerVolume:),0.0f);
        if([obj respondsToSelector:@selector(mute)]){((void(*)(id,SEL))objc_msgSend)(obj,@selector(mute));touched=YES;}
    }
    if(touched) NSLog(@"[TikTokPlus] private audio object %@ muted=%@",NSStringFromClass([obj class]),gPrivateTikTokMuted?@"YES":@"NO");
}

static void PTScanObject(id root, NSInteger depth) {
    if(!root||depth>4)return;
    PTMuteObject(root);
    Class cls=object_getClass(root); if(!cls)return;
    unsigned int count=0; Ivar *ivars=class_copyIvarList(cls,&count);
    for(unsigned int i=0;i<count;i++){
        const char *type=ivar_getTypeEncoding(ivars[i]); if(!type||type[0]!='@')continue;
        id child=object_getIvar(root,ivars[i]); if(!child||child==root)continue;
...
- (void)playerWillLoopPlaying:(id)player { %orig; if(PTIsTikTok())PTApplyToPlayer(player); }
- (void)play {
    if(PTIsTikTok()&&gPrivateTikTokMuted){ id p=PTObject(self,@selector(player)); if(!p)p=PTObject(self,@selector(currentPlayer)); PTApplyToPlayer(p); PTMuteObject(self); }
    %orig;
    if(PTIsTikTok()&&gPrivateTikTokMuted){ id p=PTObject(self,@selector(player)); if(!p)p=PTObject(self,@selector(currentPlayer)); PTApplyToPlayer(p); PTMuteObject(self); }
}
...
- (void)containerDidFullyDisplayWithReason:(NSInteger)reason { %orig; if(PTIsTikTok()&&gPrivateTikTokMuted)PTScanObject(self,0); }
- (void)playerWillLoopPlaying:(id)player { %orig; if(PTIsTikTok())PTApplyToPlayer(player); }
%end

static void PTMuteCurrentUI(void) {
    if(!PTIsTikTok()||!gPrivateTikTokMuted)return;
    dispatch_async(dispatch_get_main_queue(),^{
        for(UIWindowScene *scene in UIApplication.sharedApplication.connectedScenes){
            if(scene.activationState!=UISceneActivationStateForegroundActive)continue;
            for(UIWindow *window in scene.windows) if(!window.hidden&&window.rootViewController) PTScanObject(window.rootViewController,0);
        }
    });
}
...
%ctor {
    ...
    dispatch_async(dispatch_get_main_queue(),^{
        [[NSNotificationCenter defaultCenter]addObserverForName:@"TikTokPlusMuteChanged" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note){
            ...
            NSLog(@"[TikTokPlus] PRIVATE MUTE BRIDGE %@",gPrivateTikTokMuted?@"ON":@"OFF");
            PTMuteCurrentUI();
        }];
    });
}
